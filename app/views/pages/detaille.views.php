...
<div class="max-w-4xl mx-auto py-10 px-4">
    <article class="bg-white rounded-xl shadow p-6">
        ...
        <div class="flex items-center gap-4">
            <form action="/like" method="POST">
                ...
                <button type="submit" class="flex items-center gap-2 bg-red-100 text-red-600 px-4 py-2 rounded-lg hover:bg-red-200 transition">❤️ Like</button>
            </form>
    ...
    <section class="mt-10 bg-white rounded-xl shadow p-6">
        ...
        <div class="border-b pb-4 mb-4">
            <p class="text-sm font-medium text-gray-700"><?= $c['owner'] ?> , <?= $c['date_comment'] ?></p>
            <p class="text-gray-600 text-sm mt-1"><?= $c['comment']?></p>
        </div>
        <?php endforeach; ?>
            <label class="block text-sm font-medium text-gray-700 mb-2">Ajouter un commentaire</label>
        <form class="mt-6" action="/comment" method="POST">
